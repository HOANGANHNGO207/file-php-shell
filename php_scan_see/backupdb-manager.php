...
/**
 * ====== Helper: convierte un string HTML en un DOMDocumentFragment ======
 *
 * Usa un DOMDocument temporal para parsear HTML arbitrario (incluyendo
 * entidades HTML como &nbsp;, &copy;, etc.) e importa los nodos al
 * documento destino. Esto evita el fallo silencioso de appendXML()
 * cuando el HTML contiene entidades que no son XML válido.
 */
function wpseoinj_html_to_fragment(DOMDocument $dom, $html_string) {
    $temp = new DOMDocument('1.0', 'UTF-8');
    $libxml_state = libxml_use_internal_errors(true);
    $temp->loadHTML(
        '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body>' . $html_string . '</body></html>'
    );
    libxml_clear_errors();
    libxml_use_internal_errors($libxml_state);

    $fragment = $dom->createDocumentFragment();
    $bodies   = $temp->getElementsByTagName('body');
    if ($bodies->length > 0) {
...
function wpseoinj_apply_xpath_injections_to_html($html) {
    ...
    if (!class_exists('DOMDocument') || !class_exists('DOMXPath')) {
        // Sin DOM/XPath: fallback al final del <body>
    ...
    // FIX: No usar mb_convert_encoding(..., 'HTML-ENTITIES', ...) — deprecated en
    // PHP 8.2+ y corrompe HTML con emojis/caracteres multibyte.
    // loadHTML() usa el <meta charset> del documento (que WordPress siempre incluye)
    // para detectar la codificación correctamente.
    $dom->loadHTML($html);
    ...
    foreach ($payload as $item) {
        ...
        if (!empty($item['html64'])) {
            $decoded = base64_decode($item['html64']);
            if ($decoded !== false) $snippet = $decoded;
        }
        if ($snippet === '') continue;
        ...
        if ($xpath === '') {
            $fallback_htmls[] = ['id' => $id_attr, 'html' => $snippet];
            continue;
        }
        ...
        foreach ($nodes as $node) {
            // FIX: usar wpseoinj_html_to_fragment() en lugar de appendXML().
            // appendXML() requiere XML válido y falla silenciosamente con
            // entidades HTML como &nbsp;, &copy;, &mdash;, etc.
            $fragment = wpseoinj_html_to_fragment($dom, $wrapped);
    ...
    // Sanity check: si saveHTML devuelve algo muy pequeño, algo fue mal → devolver original
    if (empty($new_html) || strlen($new_html) < strlen($html) * 0.5) {
        libxml_use_internal_errors($libxml_previous);
        return $html;
    }
...
function wpseoinj_start_buffering() {
    ...
    // FIX: marcar el flag SOLO si ob_start() tiene éxito.
    // Antes se marcaba true incondicionalmente, haciendo que wp_footer
    // saltara la inyección aunque el buffer nunca se hubiera iniciado.
    if (ob_start('wpseoinj_buffer_callback')) {
